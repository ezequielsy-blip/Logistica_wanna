...
# --- TU LLAVE JSON (Corregida para evitar errores de formato) ---
CREDS_DICT = {
...
# --- FUNCION SUBIR A DRIVE (Corregida con método Create) ---
def subir_a_drive():
    try:
        ...
        # Primero intentamos actualizar el archivo por su ID
        try:
            service.files().update(fileId=FILE_ID, media_body=media).execute()
            st.success("✅ CAMBIOS SUBIDOS A DRIVE EXITOSAMENTE (Update)")
        except:
            # Si el Update falla (404), creamos uno nuevo para no perder datos
            file_metadata = {'name': 'inventario_wms.db'}
            service.files().create(body=file_metadata, media_body=media, fields='id').execute()
            st.success("✅ CAMBIOS SUBIDOS COMO ARCHIVO NUEVO EN DRIVE")
