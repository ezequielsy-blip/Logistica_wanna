...
# Inicialización crítica para que "PASAR" funcione entre pestañas
if "transfer_data" not in st.session_state:
    st.session_state.transfer_data = None

# --- ESTILOS COMPLETOS (Modo oscuro, botones gigantes y banners) ---
st.markdown("""
    <style>
...
# --- MOTORES DE LÓGICA (Ubicaciones sugeridas) ---
...
with st.sidebar:
    ...
    if u_log == "admin" and p_log == "70797474":
        ...
        try:
            ...
            st.error("Error: Acceso restringido por RLS.")
...
# --- TAB ENTRADAS (Buscador Exacto + Auto-relleno de PASAR) ---
with t1:
    if es_autorizado:
        # Recuperar valor si viene de un "PASAR"
        val_pasado = str(st.session_state.transfer_data['cod_int']) if st.session_state.transfer_data else ""
        
        bus = st.text_input("🔍 ESCANEAR/BUSCAR PRODUCTO", value=val_pasado, key="ent_bus")
        if bus:
            if bus.isdigit():
            ...
            if m_data:
                if len(m_data) > 1:
                    ...
                    p_sel = st.selectbox("Seleccione el producto correcto:", list(opciones_p.keys()))
                    p = opciones_p[p_sel]
                else:
                    p = m_data[0]
                ...
                with st.form("f_carga", clear_on_submit=True):
                    ...
                    c1, c2 = st.columns(2)
                    q = c1.number_input("CANTIDAD", min_value=1, step=1, value=q_v)
...
# --- TAB STOCK / PASES (Incluye EDITAR LOTE) ---
with t2:
                ...
                with st.container():
                    ...
                    if es_autorizado:
                        ...
                        if c_pas.button("PASAR", key=f"bpa_{r['id']}"):
                            ...
                            st.success("Cargado. Ve a ENTRADAS.")

# --- TAB PLANILLA ---
with t3:
    p = supabase.table("inventario").select("*").order("id", desc=True).execute()
    if p.data:
        st.dataframe(pd.DataFrame(p.data), use_container_width=True, hide_index=True)

# --- TAB USUARIOS (Persistencia en Nube) ---
if es_admin_maestro:
    with t_extra[0]:
            ...
            if st.form_submit_button("➕ REGISTRAR"):
                if n_u and n_p:
                    try:
                        ...
                        st.error("El usuario ya existe o hay un error de RLS.")
