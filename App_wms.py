...
# --- LLAVE DE SEGURIDAD (Formato corregido para evitar error PEM) ---
CREDS_DICT = {
...
def subir_a_drive():
    try:
        ...
        st.success("✅ CAMBIOS SUBIDOS A DRIVE")
...
with tab1:
    if not es_autorizado: st.warning("🔒 Ingrese clave.")
        ...
        if sel:
            ...
            c_s, n_s, b_s = item['cod_int'], item['nombre'], item['barras']
        else: c_s, n_s, b_s = "", "", ""

        with st.form("form_entrada", clear_on_submit=True):
            col1, col2 = st.columns(2)
            with col1:
                f_cod = st.text_input("Código Interno", value=c_s)
                f_nom = st.text_input("Descripción", value=n_s)
            ...
            if st.form_submit_button("⚡ REGISTRAR"):
                if f_cod and len(f_venc_raw) == 4:
                    ...
                    cursor.execute("INSERT INTO inventario VALUES (?,?,?,?,?,?,?)", (f_cod, f_can, f_nom, b_s, f_venc, f_ubi, f_dep))
                    conn.commit(); st.success("Guardado"); st.rerun()

with tab2:
    bus_d = st.text_input("🔍 Buscar Stock", key="tab2_bus")
    if bus_d:
