...
with tab1:
    ...
    try:
        # Búsqueda elástica igual que en PC
...
# --- TAB 2: DESPACHO (SOLUCIÓN AL ERROR DE BÚSQUEDA) ---
with tab2:
    st.subheader("Salida de Mercadería")
    bus = st.text_input("🔍 Buscar para Despacho", key="in_des")
    if bus:
        # Aquí igualamos la potencia de búsqueda a la de Movimientos
        query_d = "SELECT rowid, * FROM inventario WHERE (cod_int LIKE ? OR nombre LIKE ? OR barras LIKE ?) AND cantidad > 0"
        res = pd.read_sql(query_d, conn, params=(f'%{bus}%', f'%{bus}%', f'%{bus}%'))
