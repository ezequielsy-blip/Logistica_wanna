...
def subir_a_drive():
    try:
        ...
        # Intenta actualizar el archivo existente
        service.files().update(fileId=FILE_ID, media_body=media).execute()
        st.success("✅ CAMBIOS GUARDADOS EN DRIVE")
    except Exception as e:
        st.error(f"❌ ERROR: {e}")
        st.info("Asegurate de que el archivo esté compartido como EDITOR con el mail del cartel de seguridad.")
...
with st.sidebar:
    ...
    if es_autorizado: st.success("MODO EDICIÓN: ACTIVADO")
    else: st.info("MODO CONSULTA")
...
# --- TAB 1: ENTRADAS ---
with tab1:
    if not es_autorizado: st.warning("🔒 Ingrese clave para operar.")
    else:
        ...
        try:
            ...
            cod_sel = seleccion.split(" | ")[0] if seleccion else ""
            nom_sel = maestra_df[maestra_df['cod_int'] == cod_sel]['nombre'].values[0] if cod_sel else ""
            bar_sel = maestra_df[maestra_df['cod_int'] == cod_sel]['barras'].values[0] if cod_sel else ""
