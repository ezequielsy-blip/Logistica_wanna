...
if _show("🔐 ADMIN"):
    if rol != "admin":
        ...
        if usuarios_db:
            for _u in usuarios_db:
                _uc1, _uc2, _uc3, _uc4 = st.columns([3, 2, 2, 1])
                with _uc1:
                    st.markdown(f'<div style="padding:10px 0;font-weight:700;color:#F1F5F9">'
                                f'👤 {_u["usuario"]}</div>', unsafe_allow_html=True)
                with _uc2:
                    _roles = ["operario","admin","visita","vendedor"]
                    _ri = _roles.index(_u["rol"]) if _u["rol"] in _roles else 0
                    _new_rol = st.selectbox("", _roles, index=_ri,
                                            key=f"rol_{_u['usuario']}",
                                            label_visibility="collapsed")
                    if _new_rol != _u["rol"]:
                        try:
                            sb.table("usuarios").update({"rol": _new_rol}).eq("usuario", _u["usuario"]).execute()
                            st.success(f"✅ Rol de {_u['usuario']} → {_new_rol}")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Error: {e}")
                with _uc3:
                    _new_clave = st.text_input("", placeholder="Nueva clave",
                                               key=f"clave_{_u['usuario']}",
                                               label_visibility="collapsed",
                                               type="password")
                    if _new_clave.strip():
                        if st.button("💾", key=f"saveclave_{_u['usuario']}"):
                            try:
                                sb.table("usuarios").update({"clave": _new_clave.strip()}).eq("usuario", _u["usuario"]).execute()
                                st.success(f"✅ Clave de {_u['usuario']} actualizada.")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error: {e}")
                with _uc4:
                    st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
                    if _u["usuario"] != usuario:  # no borrar al usuario activo
                        if st.button("🗑️", key=f"del_{_u['usuario']}", help=f"Eliminar {_u['usuario']}"):
                            try:
                                sb.table("usuarios").delete().eq("usuario", _u["usuario"]).execute()
                                st.success(f"✅ {_u['usuario']} eliminado.")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error: {e}")
        ...
        st.markdown('<p class="sec-label">⚙️ CONFIGURACIÓN GENERAL</p>', unsafe_allow_html=True)
        ...
        if _cfg_tab == "🗄️ Estanterías":
            ...
            for _i, _e in enumerate(_nueva_est):
                ...
                with _ec[4]:
                    ...
                    if st.button("🗑", key=f"edel_{_e['num']}"):
                        ...
                        _nueva_cfg2 = dict(_cfg_admin); _nueva_cfg2["estantes"] = _nueva_est
                        if _guardar_cfg_admin(_nueva_cfg2): st.success("✅ Estante eliminado."); st.rerun()
            ...
            with st.expander("➕ Agregar estante"):
                ...
                if st.button("➕ Agregar estante", use_container_width=True, key="eadd"):
                    if any(x["num"]==_new_num for x in _nueva_est):
                        ...
                        _nueva_cfg2 = dict(_cfg_admin); _nueva_cfg2["estantes"] = _nueva_est
                        if _guardar_cfg_admin(_nueva_cfg2): st.success(f"✅ Estante {_new_num} agregado."); st.rerun()

            if st.button("💾 Guardar estanterías", use_container_width=True, type="primary", key="egardar"):
                _nueva_cfg2 = dict(_cfg_admin); _nueva_cfg2["estantes"] = _nueva_est
                if _guardar_cfg_admin(_nueva_cfg2): st.success(f"✅ {len(_nueva_est)} estantes guardados."); st.rerun()
        ...
        # ════════════════════════════════════════════════════════════════
            ...
            _gc1, _gc2 = st.columns(2)
            with _gc1:
                _dias_val = int(_cfg_admin.get("dias_alerta", 60))
                _dias_new = st.number_input("⏰ Días de alerta por vencimiento", min_value=1, max_value=365, value=_dias_val, key="cfg_dias")
            with _gc2:
                _hist_val = int(_cfg_admin.get("hist_limit", 500))
                _hist_new = st.number_input("📋 Límite de registros en historial", min_value=50, max_value=5000, step=50, value=_hist_val, key="cfg_hist")
            _gc3, _gc4 = st.columns(2)
            with _gc3:
                _deps = ["depo 1","depo 2","depo 3"]
                _dep_val = _cfg_admin.get("depo_default","depo 1")
                _dep_idx = _deps.index(_dep_val) if _dep_val in _deps else 0
                _dep_new = st.selectbox("🏭 Depósito por defecto", _deps, index=_dep_idx, key="cfg_dep")
            with _gc4:
                _res_val = int(_cfg_admin.get("max_resultados", 100))
                _res_new = st.number_input("🔍 Máximo resultados búsqueda", min_value=10, max_value=500, step=10, value=_res_val, key="cfg_res")
            if st.button("💾 Guardar configuración general", use_container_width=True, type="primary", key="cfg_gen_save"):
                _nueva_cfg2 = dict(_cfg_admin)
                _nueva_cfg2.update({"dias_alerta":_dias_new,"hist_limit":_hist_new,"depo_default":_dep_new,"max_resultados":_res_new})
                if _guardar_cfg_admin(_nueva_cfg2): st.success("✅ Configuración guardada."); st.rerun()

        # ════════════════════════════════════════════════════════════════
        # SUB-TAB: APARIENCIA (colores + paletas)
        # ════════════════════════════════════════════════════════════════
        elif _cfg_tab == "🎨 Apariencia":
            _PALETAS = {
                "🌌 Midnight Pro":  {"bg":"#04080F","surface":"#0A1628","surface2":"#0F1E35","primary":"#4D7CFE","primary_dark":"#2D5BE3","accent":"#7EB8F7","success":"#56CFE1","warning":"#FFD166","danger":"#EF476F","text":"#E8F4FD","text_dim":"#4A6FA5","border":"#152340"},
            ...
            _colores_actuales = _cfg_admin.get("colores", {
                "bg":"#0F172A","surface":"#1E293B","surface2":"#263347",
                "primary":"#3B82F6","primary_dark":"#1D4ED8","accent":"#06B6D4",
                "success":"#10B981","warning":"#F59E0B","danger":"#EF4444",
                "text":"#F1F5F9","text_dim":"#94A3B8","border":"#334155"
            })

            st.markdown("**🎨 Paletas rápidas** — aplicá un tema completo con un clic:")
            _pcols = st.columns(5)
            for _pi, (_pnom, _pcols_d) in enumerate(_PALETAS.items()):
                _col = _pcols[_pi % 5]
                _pbg = _pcols_d.get("surface","#1E293B")
                _ppr = _pcols_d.get("primary","#3B82F6")
                _col.markdown(f'<div style="background:{_pbg};border:2px solid {_ppr};border-radius:10px;padding:8px;text-align:center;font-size:11px;font-weight:700;color:{_pcols_d.get("text","#fff")};margin-bottom:4px">{_pnom}</div>', unsafe_allow_html=True)
                if _col.button("Aplicar", key=f"pal_{_pi}", use_container_width=True):
                    _nueva_cfg2 = dict(_cfg_admin); _nueva_cfg2["colores"] = _pcols_d
                    if _guardar_cfg_admin(_nueva_cfg2):
                        st.success(f"✅ Paleta {_pnom} aplicada. Recargá la página para ver los cambios.")
                        st.rerun()

            st.markdown("---")
            st.markdown("**🖌️ Colores personalizados:**")
            _COLOR_DEFS = [
                ("bg",           "Fondo de ventana",      "🖥️"),
            ...
            for _ck, _cl, _ico in _COLOR_DEFS:
                _cv = _colores_actuales.get(_ck, "#000000")
                _ccol1, _ccol2, _ccol3 = st.columns([1,3,2])
                _ccol1.markdown(f'<div style="background:{_cv};width:32px;height:32px;border-radius:8px;border:2px solid #334155;margin-top:4px"></div>', unsafe_allow_html=True)
                _ccol2.markdown(f'<div style="padding:8px 0;font-size:13px;font-weight:600;color:#F1F5F9">{_ico} {_cl}</div>', unsafe_allow_html=True)
                _nv = _ccol3.text_input("", value=_cv, key=f"color_{_ck}", label_visibility="collapsed", placeholder="#rrggbb")
                if _nv.strip() and _nv.strip() != _cv and len(_nv.strip())==7 and _nv.strip().startswith("#"):
                    _cols_nuevos[_ck] = _nv.strip()

            if st.button("💾 Guardar colores personalizados", use_container_width=True, type="primary", key="colores_save"):
                _nueva_cfg2 = dict(_cfg_admin); _nueva_cfg2["colores"] = _cols_nuevos
                if _guardar_cfg_admin(_nueva_cfg2):
                    st.success("✅ Colores guardados. Recargá la página para aplicarlos.")
                    st.rerun()
...
if _show("🤖 ASISTENTE"):
    ...
    # ── Input oculto con on_change: postMessage → widget → _lz_pending ────────
    def _on_hidden_change():
        v = st.session_state.get("_bot_hidden_input","").strip()
        if v:
            st.session_state["_lz_pending"] = v

    _stc.html("""<script>
    window.addEventListener("message",function(e){
      if(!e.data||e.data.type!=="lz_msg") return;
      var inp=Array.from(window.parent.document.querySelectorAll('input[type="text"]')).find(function(el){return el.placeholder==="_lzhidden_";});
      if(inp){
        Object.getOwnPropertyDescriptor(window.parent.HTMLInputElement.prototype,'value').set.call(inp,e.data.text);
    ...
    _lz_msg = st.session_state.pop("_lz_pending", "")
...
function closeScan(){
  ...
  M("vid").srcObject=null;M("scbtn").className="ibtn ibtn-scan";M("ov").className="";
    ...
    if _final:
        st.session_state.pop("_lz_pending", None)
